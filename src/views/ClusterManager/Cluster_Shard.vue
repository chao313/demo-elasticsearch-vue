<template>
    <div class="app-container">
        <div class="mt20">
            <el-form :inline="true" size="mini">
                ...
                <el-form-item label="ipContain">
                    <el-input v-model="headers.ES_FILTER.ip" placeholder="请输入ip"></el-input>
                </el-form-item>
                ...
                <el-form-item label="stateContain">
                    <el-input v-model="headers.ES_FILTER.state"
                              placeholder="请输入stateContain(STARTED/UNASSIGNED)"></el-input>
                </el-form-item>
                <el-form-item>
        ...
        <div class="app-list">
            <div class="app-tab">
                <hr>
                <h5 class="form-tit">集群Shards信息</h5>
                <table>
                    <thead>
                    <tr>
                        <th>id</th>
                    ...
                    <tbody>
                    <template v-if="Cluster_ShardController_Cat_Shards_Result.list">
                        <tr v-for="(info,index) in Cluster_ShardController_Cat_Shards_Result.list">
                            <td>{{index+1}}</td>
                            <td>{{info.ip}}</td>
                            <td>{{info.node}}</td>
                            <td>{{info.index}}</td>
        ...
        <div class="mt10">
            <!--/** */:page-size  数一页的数量！！！-->
            <el-pagination v-show="Cluster_ShardController_Cat_Shards_Result.list.length > 0"
                           background
                           @current-change="handleCurrentChange"
                           :current-page.sync="Cluster_ShardController_Cat_Shards_Result.pageNum"
                           :page-size="Cluster_ShardController_Cat_Shards_Result.pageSize"
                           layout="total, prev, pager, next, jumper"
                           :total="Cluster_ShardController_Cat_Shards_Result.total">
            </el-pagination>
        </div>
    </div>
</template>
        ...
        methods: {
            ...
            handleCurrentChange(currentChange) {
                ...
                self.$http.get(self.api.RedisController_GetRecordByScrollId, {
                    params: {
                        'scrollId': self.Cluster_ShardController_Cat_Shards_Result.orderBy,
                        'pageNum': currentChange,
                        'pageSize': self.Cluster_ShardController_Cat_Shards_Result.pageSize,
                    }
                }, function (response) {
                    if (response.code == 0) {
                        self.Cluster_ShardController_Cat_Shards_Result = response.content;
            ...
            searchEvent() {
                this.Cluster_ShardController_Cat_Shards();
            },
